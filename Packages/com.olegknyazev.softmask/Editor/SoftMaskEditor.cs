...
namespace SoftMasking.Editor
{
    ...
    public class SoftMaskEditor : UnityEditor.Editor
    {
        ...
        static class Labels
        {
            ...
            public static readonly GUIContent SpritePixelsPerUnitMultiplier = new GUIContent(
                ...
                switch ((SoftMask.MaskSource)_source.enumValueIndex)
                {
                    ...
                    case SoftMask.MaskSource.Sprite:
                        ...
                        if ((SoftMask.BorderMode)_spriteBorderMode.enumValueIndex != SoftMask.BorderMode.Simple)
                            EditorGUILayout.PropertyField(_spritePixelsPerUnitMultiplier,
                                Labels.SpritePixelsPerUnitMultiplier);
                        break;
        ...
        void ShowErrorsIfAny()
        {
            ...
            ShowErrorIfPresent(errors, SoftMask.Errors.UnsupportedShaders, Labels.UnsupportedShaders,
                MessageType.Warning);
            ...
            ShowErrorIfPresent(errors, SoftMask.Errors.UnsupportedImageType, Labels.UnsupportedImageType,
                MessageType.Error);
            ShowErrorIfPresent(errors, SoftMask.Errors.UnreadableTexture, Labels.UnreadableTexture, MessageType.Error);
            ShowErrorIfPresent(errors, SoftMask.Errors.UnreadableRenderTexture, Labels.UnreadableRenderTexture,
                MessageType.Error);
        }
        ...
        {
            public static bool ChannelWeights(GUIContent label, SerializedProperty weightsProp,
                bool customWeightsExpanded)
            {
            ...
            static bool ChannelWeights(Rect rect, GUIContent label, SerializedProperty weightsProp,
                bool customWeightsExpanded)
            {
                var knownChannel =
            ...
            {
                var result = rect;
                result.y += HeightOf(KnownChannelStyle) +
                            Mathf.Max(KnownChannelStyle.margin.bottom, CustomWeightsStyle.margin.top);
                result.height = HeightOf(CustomWeightsStyle);
                return result;
            }

            static float HeightOf(GUIStyle style)
            {
                return style.CalcSize(GUIContent.none).y;
            }
